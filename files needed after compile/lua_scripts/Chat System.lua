...
local showLogin			= false -- Sort of a debug setting (Shows status on login)
local saveMethod 		= {account = true, character = false} -- if both true, account has highest priority
local defaultSettings 	= {'EN', true, false, false} -- {language, global, pvp, pve}
...
local messages = { -- All the messages in a language needs a string.
	...
	['DK'] = {
		...
		help = {'|PTC|Chattene er opdelt i tre chatrum: Global, PvP og PvE.',
				'|PTC|Du kan slå dem til og fra når du har lyst.',
				'|PTC|Hvis den valgte chat er slået til, kan du sende og modtage bedskeder.',
				'|PTC|Hvis den valgte chat er slået fra, det modsatte...'
				},
		globalchat = { 
			channel = '|PTP||PTC|[G]|PGMC|[|PGMR|]|PCC|[|P|]|PTC| siger: ',
			disabled = 'Global chatten skal være slået til.'
		},
		pvpchat = { 
			channel = '|PTP||PTC|[PVP]|PGMC|[|PGMR|]|PCC|[|P|]|PTC| siger: ',
			disabled = 'PvP chatten skal være slået til.'
		},
		pvechat = { 
			channel = '|PTP||PTC|[PVE]|PGMC|[|PGMR|]|PCC|[|P|]|PTC| siger: ',
			disabled = 'PvE chatten skal være slået til.'
		}
	}
}
